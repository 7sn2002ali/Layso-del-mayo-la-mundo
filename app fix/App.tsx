...
export default function App() {
  ...
  const handleSeedData = async () => {
     ...
     if(mockItems.length > 0) {
        ...
        // Check if we used fallback data (checking if ID is FB- or ST- prefix usually, but here we just log success)
        if (mockItems[0].sku.startsWith('ST-')) {
            addLog('INBOUND', `Imported ${placed} STATIC mock pallets (AI Offline).`);
        } else {
            addLog('INBOUND', `AI Generated & Saved ${placed} mock pallets.`);
        }
     } else {
         addLog('ERROR', 'Failed to generate data.');
     }
     setLoadingAI(false);
  }
